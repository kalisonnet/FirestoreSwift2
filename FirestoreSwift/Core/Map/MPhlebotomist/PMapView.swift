...
struct PMapView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> MKMapView {
        ...
        // Fetch and display physician locations for orders assigned to the logged-in user
        if let userId = loggedInUserId {
            orderManager.fetchOrdersForLoggedInUser {
                // Fetch physician coordinates for assigned orders
                orderManager.fetchPhysicianCoordinatesForLoggedInUser { annotations in
                    mapView.addAnnotations(annotations)
                    print("Added \(annotations.count) physician annotations for assigned orders to the map")
