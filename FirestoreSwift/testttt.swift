...
struct testtsPMOrderListView: View {
    ...
    var filteredOrders: [Order] {
            if searchText.isEmpty {
                ...
                return orderManager.orders.filter { $0.patient_name.lowercased().contains(searchText.lowercased()) }
            }
        }
    ...
    var body: some View {
        NavigationView {
            ...
            .sheet(isPresented: $showStatusSelection) { // Show full-screen StatusSelectionView
                            StatusSelectionView(
    ...
    private func updateOrderStatus(status: String) {
        ...
        selectedOrders.forEach { orderId in
            if var order = orderManager.orders.first(where: { $0.id == orderId }) {
                // Append the new status with the timestamp
                let statusEntry = OrderStatus(status: status, timestamp: Date())
                order.status.append(statusEntry) // Append the new status entry
                orderManager.updateOrder(order)
            }
        }
    }
